...
from auth_helpers import get_bearer_token, get_db, verify_firebase_token
from entitlements import require_pro_or_business
from usage_caps import get_tier_and_status


def require_campaign_intelligence_user(
    ...
    if not is_admin:
        ...
        if status not in {"active", "trialing"}:
            raise HTTPException(
                status_code=402,
                detail="An active Pro or Business subscription is required for Campaign Intelligence.",
            )
